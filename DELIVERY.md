...
| Web ページの画像 | 画像を右クリック → "画像のアドレスをコピー" → ブラウザで `https://odayakalife.dev/check-value-app/?img=<貼付>` を開く | 3 |
...
## S2: Web ページの画像 — ブックマークレット ✅

### 状況
Chrome / Safari / Firefox で Web ページを見ていて、表示されている画像のヴァルールを確認したい。Pinterest の個別ピン詳細ページ、Wikipedia の記事中の絵画画像、ブログ記事の挿絵など。

### 操作 (初回1回)
1. 以下のページをブラウザで開く: <https://odayakalife.dev/check-value-app/bookmarklet.html>
2. ページ内の「▶ ヴァルール確認」ボタンを **ブックマークバーにドラッグ**
3. （ドラッグできなければ、同ページの「↑をコピー」ボタンで JS をコピーし、適当なページをブックマーク → 編集 → URL に貼り付け、名前を「ヴァルール確認」に）

### 操作 (毎回)
1. 確認したい画像が表示されているページで、ブックマークバーの「ヴァルール確認」をクリック

### 結果
新規タブで本ツールが開き、そのページで一番大きい `<img>` のヴァルール変換結果が即表示される。

### 仕組み / 根拠
- ブックマークレットの本体は `javascript:` スキームの URL。ブラウザは現在ページの window context で JS を実行する（[MDN: javascript: URLs](https://developer.mozilla.org/en-US/docs/Web/URI/Reference/Schemes/javascript)）
- JS は `document.images` を取得し、`naturalWidth × naturalHeight` 最大のものを選び `currentSrc` で URL を得て、本ツールに `?img=` で渡す
- 本ツール側は `index.html` 内のブート処理で `?img=<URL>` を `<img crossOrigin="anonymous" src="...">` として読み込む

### 動かない条件
- ❌ 画像が `<img>` でなく CSS `background-image` で配置されている（Pinterest のグリッド一覧画面、Instagram など）→ 画像個別ページに遷移してから起動
- ❌ サイトの Content Security Policy (CSP) が `javascript:` URL をブロックしている（X (Twitter) で報告あり）→ 画像を別タブで直接開いてから起動
- ⚠️ 取得した画像URLが CORS を許可していない場合、本ツール側で読み込みエラー（ブラウザコンソールに `CORS` エラー）。多くの CDN は許可するが、要ログインの認証画像は不可

### 配布
[bookmarklet.html](./bookmarklet.html)（GitHub Pages 上にホストされている、ドラッグ可能リンクと手動設定手順あり）
